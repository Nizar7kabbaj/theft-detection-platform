...
_stub: AuditServiceStub | None = None
...
def _new_event(actor: str, severity: int) -> tuple[pb.AuditEvent, datetime]:
...
def login_success(
    ...
    event, occurred_at = _new_event(subject_id, common_pb2.SEVERITY_INFO)
...
def login_failure(
    ...
    try:
        ...
        return None
    event, occurred_at = _new_event("", common_pb2.SEVERITY_WARNING)
...
def refresh_reuse_detected(
    ...
    event, occurred_at = _new_event(subject_id, common_pb2.SEVERITY_CRITICAL)
...
def throttle_triggered(
    ...
    try:
        ...
        return None
    event, occurred_at = _new_event("", common_pb2.SEVERITY_CRITICAL)
...
def admin_session_revoked(actor_user_id: str, session_id: str) -> PreparedEvent:
...
def open_audit_client() -> AuditServiceStub:
    ...
    _stub = AuditServiceStub(_channel)
    logger.info("audit client ready")
    return _stub


def audit_stub() -> AuditServiceStub | None:
    return _stub
