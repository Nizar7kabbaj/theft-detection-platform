...
Format compatibility is what decided it. PoseLift skips the entire video → keypoint preprocessing step that every other candidate requires. On a deadline-driven demo, that's the trade-off that matters.
...
The dataset was designed for unsupervised anomaly detection: train on normal behavior (Train/), then flag anomalies on Test/. Labels only exist for the Test split, by design.
...
3. **Item-value bias.** The dataset skews toward large-item theft (bottles, packaged goods). Palmed small items — chocolate bars, cosmetics — are underrepresented. Our model inherits this.
4. **Bounding-box-relative normalization is required.** Without it, the model learns "shoplifting happens at x<300" — useless on any other camera. We normalize relative to each person's bounding box at both training and inference time.
...
6. **Pose-extraction failures cascade.** HRNet produced keypoints from the raw videos; occlusion behind shelves causes NaN keypoints that need sanitizing before training (lesson #51).

---

## V2 path (post-meeting)

The upgrade is **RetailS**, from the same research group: ~20 million frames, multi-camera, semi-supervised. It solves the sample-size and single-store problems above. It also requires a real preprocessing pipeline — which is why it's V2, not V1.
